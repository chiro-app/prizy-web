...
public class GameEngine {
  ...
  public GameEvent startGame(String requestId, UUID userId, GameEvent.GameStarted event) {
    if (activeContexts.get().containsKey(requestId)) {
      return new GameEvent.Error(Code.GANE_ALREADY_STARTED);
    }

    var user = userRepository.byId(userId).get();
    var maybeContest = contestRepository.byId(event.contestId());
    if (maybeContest.isEmpty()) {
      return new GameEvent.Error(Code.CONTEST_NOT_FOUND);
    }
    ...
    if (balance.diamonds() < event.diamonds()) {
      return new GameEvent.Error(Code.INSUFFICIENT_RESOURCES);
    }
    if (balance.lives() < 1) {
      return new GameEvent.Error(Code.INSUFFICIENT_RESOURCES);
    }
  ...
  public GameEvent movePlayer(String requestId, UUID userId, GameEvent.PlayerMoved event) {
